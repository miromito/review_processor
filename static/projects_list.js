(() => {
  ...
  function projectStatusIndicator(p) {
    const tip = esc(phaseRu[p.phase] || p.phase);
    if (p.phase === "analyzing") {
      return `<div class="ra-project-status" role="status" title="${tip}" aria-label="${tip}">
        <span class="spinner-border spinner-border-sm text-primary" aria-hidden="true"></span>
        <span class="visually-hidden">${tip}</span>
      </div>`;
    }
    if (p.phase === "complete") {
      return `<div class="ra-project-status text-success" title="${tip}" role="img" aria-label="${tip}">
    ...
    if (p.phase === "error") {
      return `<div class="ra-project-status text-danger" title="${tip}" role="img" aria-label="${tip}">
        <svg width="18" height="18" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true" focusable="false">
    ...
    if (p.phase === "awaiting_file" || p.phase === "awaiting_mapping" || p.phase === "awaiting_analysis") {
      return `<div class="ra-project-status text-warning" title="${tip}" role="img" aria-label="${tip}">
        <svg width="18" height="18" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true" focusable="false">
    ...
    return `<span class="text-muted ra-project-status" title="${tip}" role="img" aria-label="${tip}">—</span>`;
  }
